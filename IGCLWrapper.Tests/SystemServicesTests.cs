...
namespace IGCLWrapper.Tests
{
    public class SystemServicesTests : IDisposable
    {
        ...
        public void GetDeviceProperties_ShouldReturnValidProperties()
        {
            ...
            var adaptersPtr = IGCL.new_deviceAdapterHandleP();
            
            ctl_result_t result = IGCL.ctlEnumerateDevices(_apiHandle, countPtr, adaptersPtr);
            Assert.Equal(ctl_result_t.CTL_RESULT_SUCCESS, result);

            // Get first adapter (IntPtr is a value type, check for IntPtr.Zero instead)
            var firstAdapter = IGCL.deviceAdapterHandleP_value(adaptersPtr);
            ...
            IGCL.delete_deviceAdapterHandleP(adaptersPtr);
            IGCL.delete_igcl_uint32P(countPtr);
        }
